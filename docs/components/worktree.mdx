...
<Worktree path="/tmp/smithers/wt-a">
  <Task id="build" output={outputC}>{{ value: 1 }}</Task>
  <Task id="test" output={outputC}>{{ value: 2 }}</Task>
  <MergeQueue>
    <Task id="apply" output={outputC}>{{ value: 3 }}</Task>
  </MergeQueue>
  <Parallel maxConcurrency={2}>
    <Task id="lint" output={outputC}>{{ value: 4 }}</Task>
  </Parallel>
  <Task id="package" output={outputC}>{{ value: 5 }}</Task>
  <Task id="release" output={outputC}>{{ value: 6 }}</Task>
</Worktree>
```
...
<Worktree id="outer" path="/tmp/wt-outer">
  <Task id="a" output={outputC}>{{ value: "outer" }}</Task>
  <Worktree id="inner" path="./nested">
    <Task id="b" output={outputC}>{{ value: "inner" }}</Task>
  </Worktree>
</Worktree>
```
...
<Worktree path="/tmp/wt-queue">
  <MergeQueue>
    {prs.map((pr) => (
      <Task key={pr.id} id={`apply-${pr.id}`} output={outputC}>
        {{ value: pr.id }}
      </Task>
    ))}
  </MergeQueue>
