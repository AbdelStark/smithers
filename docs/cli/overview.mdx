...
| `--allow-network` | Allow the `bash` tool to make network requests. |
...
Render the workflow tree without executing, and print the `GraphSnapshot` as JSON. Uses [renderFrame](/docs/runtime/render-frame) internally.
...
Revert the workspace to a previous task attempt's filesystem state using JJ (Jujutsu). See [Revert](/docs/runtime/revert) for full details.
...
| `--allow-network` | Allow the `bash` tool to make outbound network requests. | Disabled |
...
- [runWorkflow](/docs/runtime/run-workflow) -- Programmatic API equivalent.
- [Events](/docs/runtime/events) -- Understanding the NDJSON event logs.
- [Revert](/docs/runtime/revert) -- Detailed revert documentation.
- [Approvals](/docs/concepts/approvals) -- How the approval system works.
