...
export const analyst = agent({
  ...
  system: "You are a senior code analyst. Return structured JSON.",
});

export const implementer = agent({
  model: anthropic(MODEL),
  system: "You are a senior engineer. Implement changes and return structured JSON.",
  tools: { read, grep, bash, edit, write },
});

export const reviewer = agent({
  model: anthropic(MODEL),
  system: "You are a strict code reviewer. Return structured JSON with approval status.",
  tools: { read, grep },
});
```
