...
const analyst = agent({
  ...
  system: "You are a senior code reviewer. Analyze code for bugs, security issues, and quality problems. Return structured JSON.",
});

const fixer = agent({
  model: anthropic("claude-sonnet-4-20250514"),
  system: "You are a senior engineer who writes minimal, correct fixes. Return structured JSON with a unified diff patch.",
});

const reporter = agent({
  model: anthropic("claude-sonnet-4-20250514"),
  system: "You are a technical writer. Summarize code review findings into a clear report. Return structured JSON.",
});
```
